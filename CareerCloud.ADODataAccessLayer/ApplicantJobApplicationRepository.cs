...
namespace CareerCloud.ADODataAccessLayer
{
    public class ApplicantJobApplicationRepository : IDataRepository<ApplicantJobApplicationPoco>
    {
        ...
        {
            _dbHelper.Update("insert into applicant_job_applications(id, applicant, job, application_date) values(@id, @applicant, @job, @application_date)",
                (cmd, item) =>
                {
        ...
        {
            return _dbHelper.Query("select id, applicant, job, application_date, time_stamp from applicant_job_applications",
                (reader) =>
                {
        ...
        {
            _dbHelper.Update("delete from applicant_job_applications where id = @id",
                (cmd, item) =>
                {
        ...
        {
            _dbHelper.Update("update applicant_job_applications set applicant = @applicant, job = @job, application_date = @application_date where id = @id",
                (cmd, item) =>
                {
                    cmd.Parameters.Add(new SqlParameter(parameterName: "@id", value: item.Id));
                    cmd.Parameters.Add(new SqlParameter(parameterName: "@applicant", value: item.Applicant));
